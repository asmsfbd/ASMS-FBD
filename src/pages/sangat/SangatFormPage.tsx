...
interface SangatForm {
  ...
  mobile:        string
  address:       string
  aadhaar:       string  // full 12 digits — only for new
  aadhaar_last4: string  // for display on edit
  centre:        string
}
...
export default function SangatFormPage() {
  ...
  useEffect(() => {
    ...
    // Set default centre for centre admin
  ...
  // Check for duplicate Aadhaar when user finishes entering it
  const checkAadhaar = async () => {
    if (form.aadhaar.length !== 12) return
    setCheckingDup(true)
    try {
      const last4 = form.aadhaar.slice(-4)
      // Check last 4 digits match (full Aadhaar is encrypted, we check last4 as proxy)
      ...
      if (data) {
        setDupCheck({ found: true, name: data.name, id: String(data.id) })
      } else {
        setDupCheck({ found: false })
      }
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isEdit && form.aadhaar.length !== 12) { setError('Enter full 12-digit Aadhaar number'); return }
    if (!isEdit && dupCheck?.found) { setError('This Aadhaar is already registered. See existing record.'); return }
    ...
    try {
      ...
      if (!isEdit) {
        // For new sangat — store last 4, and encrypted full Aadhaar
        // (encryption via pgcrypto happens in a DB function or we store last4 only for now)
        payload.aadhaar_last4 = form.aadhaar.slice(-4)
        // Note: full Aadhaar encryption would use a Supabase Edge Function
        // For now we store last 4 only — the policy is enforced at form level
      }
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {!isEdit && (
          <div className="bg-white rounded-xl border border-slate-200 p-4">
            ...
            />
            <p className="text-[10px] text-slate-400 mt-1">Only last 4 digits will be shown on NR prints</p>

            {checkingDup && (
              <p className="text-xs text-slate-400 mt-2">Checking for duplicates...</p>
            )}
            {dupCheck?.found && (
              <div className="flex items-start gap-2 mt-2 p-2.5 bg-amber-50 border border-amber-200 rounded-lg">
                <AlertTriangle size={14} className="text-amber-500 flex-shrink-0 mt-0.5" />
                <div>
                  <p className="text-xs font-medium text-amber-700">Possible duplicate found</p>
                  <p className="text-[10px] text-amber-600">{dupCheck.name} already registered with matching Aadhaar last 4</p>
            ...
            {dupCheck?.found === false && form.aadhaar.length === 12 && (
              <div className="flex items-center gap-2 mt-2">
                ...
                <p className="text-xs text-green-600">Aadhaar not found — new record</p>
              </div>
            )}
          </div>
        )}

        {/* Show masked Aadhaar on edit */}
        {isEdit && (
          <div className="bg-slate-50 rounded-xl p-4 border border-slate-200">
            <p className="text-xs text-slate-500 mb-1">Aadhaar Number (आधार संख्या)</p>
            <p className="font-mono text-sm text-slate-700">XXXXXXXX{form.aadhaar_last4}</p>
            <p className="text-[10px] text-slate-400 mt-0.5">Aadhaar cannot be changed after registration</p>
          </div>
        )}
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4 space-y-3">
          ...
          <div>
            <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide block mb-1">Name (नाम) *</label>
            <input
              value={form.name}
          ...
          <div>
            <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide block mb-1">Father / Husband Name (पिता / पति)</label>
            <input
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide block mb-1">Gender (लिंग) *</label>
              <select
                value={form.gender}
            ...
            <div>
              <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide block mb-1">Age (आयु)</label>
              <input
                type="number"
                inputMode="numeric"
                min={1} max={120}
          ...
          <div>
            <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide block mb-1">Mobile (मोबाइल)</label>
            <input
              type="tel"
              inputMode="numeric"
              value={form.mobile}
          ...
          <div>
            <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide block mb-1">Address (पता)</label>
            <textarea
              value={form.address}
          ...
          <div>
            <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide block mb-1">Centre (केंद्र) *</label>
            {user?.role === 'aso' ? (
              <select
                value={form.centre}
